...
| `coreai.mojo-graph-lowering` | **upstream gap** | The open-source MAX tree exposes standard graph construction but not the graph compiler/driver extension needed to register a Core AI backend. Standard `linalg.matmul[target="coreai"]` therefore fails at compile time. Do not substitute fixed semantic markers, an LLVM pattern pass, or a project-specific Mojo API. |
...
| `coreai.unsupported-graph-op` | **upstream gap** | Until a standard graph backend exists, Core AI graph lowering is rejected as a whole by name. A future backend must diagnose each unsupported operation, dtype, shape, layout, and effect during ordinary graph conversion. |
