...
## Enablement queue — known contract gaps

Work these items after the current Metal enablement queue, preserving the
public C boundary and keeping Mojo internal ABI and `raises` out of Swift.

- [ ] **Production ownership and errors**: Define and implement strings, slices, structures, handles, allocators, diagnostics, callbacks, reentrancy, Swift concurrency, and ABI versioning through the public C boundary.
